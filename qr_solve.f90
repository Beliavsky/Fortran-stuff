...
implicit none
private
public :: dqrls, svd_solve, qr_solve, normal_solve, dqrdc, normal_solve_wrap, &
          atran_a,print_reg
integer, parameter :: dp = kind(1.0d0), istdout = 6
interface normal_solve
   module procedure normal_solve_wrap, normal_solve_base
end interface normal_solve
interface qr_solve
   module procedure qr_solve_base,qr_solve_wrap
end interface qr_solve
contains
function atran_a(a) result(ata)
! compute a'a = matmul(transpose(a),a)
real(kind=dp), intent(in) :: a(:,:) 
real(kind=dp)             :: ata(size(a,2),size(a,2))
integer                   :: i,j,n2
n2 = size(a,2)
! print*,"n2,shape(ata)=",n2,shape(ata)
do i=1,n2
   do j=i,n2
      ata(i,j) = sum(a(:,i)*a(:,j))
   end do
...
!    LC: QA214.L56.
!  Parameters:
!    Input/output, real(kind=dp) A(LDA,N).  On input, the matrix whose
...
!    Input, real(kind=dp) TOL, a relative tolerance used to determine the
...
!    Output, real(kind=dp) QRAUX(N), will contain extra information defining
!    the QR factorization.
!    Workspace, real(kind=dp) WORK(N).
  integer :: lda
  integer :: n
...
!    LC: QA214.L56.
!  Parameters:
!    Input/output, real(kind=dp) A(LDA,P).  On input, the N by P matrix
...
!    Output, real(kind=dp) QRAUX(P), contains further information required
...
!    Workspace, real(kind=dp) WORK(P).  WORK is not referenced if JOB == 0.
!    Input, integer :: JOB, initiates column pivoting.
!    0, no pivoting is done.
!    nonzero, pivoting is done.
! arguments
  integer (kind = 4), intent(in)     :: lda ! leading dimension of the array A. Must be at least equal to N.
  integer (kind = 4), intent(in)     :: n   ! # of rows of the matrix A
  integer (kind = 4), intent(in)     :: p   ! # of columns of the matrix A
  real    (kind = 8), intent(in out) :: a(lda,p) ! On input, the n by p matrix whose decomposition is to be computed.
                                                 ! On output, contains in its upper triangle the upper triangular matrix R of the QR
                                                 ! factorization.  Below its diagonal A contains information from
                                                 ! which the orthogonal part of the decomposition can be recovered.
  integer (kind = 4), intent(in out) :: jpvt(p)
  real    (kind = 8), intent(out)    :: qraux(p) ! contains further information required to recover the orthogonal part of the decomposition
  real    (kind = 8), intent(out)    :: work(p)  ! workspace -- not referenced if JOB == 0
  integer (kind = 4), intent(in)     :: job      ! 0, no pivoting is done; nonzero, pivoting is done
! local variables
  integer (kind = 4) j,jp
  integer (kind = 4) l
  integer (kind = 4) lup
...
!    LC: TA345.K34.
!  Parameters:
!    Input/output, real(kind=dp) A(LDA,N), an M by N matrix.
...
!    element.
!    Output, integer :: KR, the numerical rank.
!    Input, real(kind=dp) B(M), the right hand side of the linear system.
!    Output, real(kind=dp) X(N), a least squares solution to the linear
!    system.
!    Output, real(kind=dp) R(M), the residual, B - A*X.  R may
!    overwrite B.
...
!    of the condition number of the matrix of independent columns,
!    and of R.  This estimate will be <= 1/TOL.
!    Workspace, real(kind=dp) QRAUX(N), required if ITASK = 1.
!    Workspace, real(kind=dp) WORK(N), required if ITASK = 1.
!    Input, integer :: ITASK.
...
!    15 April 2012
!  Parameters:
!    Input, real(kind=dp) A(LDA,N), the QR factorization information
...
!    by DQRANK.
!    Input, real(kind=dp) B(M), the right hand side of the linear system.
!    Output, real(kind=dp) X(N), a least squares solution to the
!    linear system.
!    Output, real(kind=dp) R(M), the residual, B - A*X.  R may
!    overwite B.
...
!    independent to within the tolerance TOL and the remaining columns
!    are linearly dependent.
!    Input, real(kind=dp) QRAUX(N), auxiliary information from DQRANK
...
!    LC: QA214.L56.
!  Parameters:
!    Input, real(kind=dp) A(LDA,P), contains the output of DQRDC.
!    Input, integer :: LDA, the leading dimension of the array A.
...
!    Input, real(kind=dp) QRAUX(P), the auxiliary output from DQRDC.
!    Input, real(kind=dp) Y(N), a vector to be manipulated by DQRSL.
!    Output, real(kind=dp) QY(N), contains Q * Y, if requested.
!    Output, real(kind=dp) QTY(N), contains Q' * Y, if requested.
!    Output, real(kind=dp) B(K), the solution of the least squares problem
!      minimize norm2 ( Y - AK * B),
!    if its computation has been requested.  Note that if pivoting was
!    requested in DQRDC, the J-th component of B will be associated with
!    column JPVT(J) of the original matrix A that was input into DQRDC.
!    Output, real(kind=dp) RSD(N), the least squares residual Y - AK * B,
!    if its computation has been requested.  RSD is also the orthogonal
!    projection of Y onto the orthogonal complement of the column space
!    of AK.
!    Output, real(kind=dp) AB(N), the least squares approximation Ak * B,
...
!  Parameters:
!    Input, integer :: N, the number of entries in the vectors.
!    Input/output, real(kind=dp) X(*), one of the vectors to be rotated.
!    Input, integer :: INCX, the increment between successive
!    entries of X.
!    Input/output, real(kind=dp) Y(*), one of the vectors to be rotated.
!    Input, integer :: INCY, the increment between successive
!    elements of Y.
!    Input, real(kind=dp) C, S, parameters (presumably the cosine and
...
!    Volume 5, Number 3, September 1979, pages 308-323.
!  Parameters:
!    Input/output, real(kind=dp) SA, SB.  On input, SA and SB are the values
!    A and B.  On output, SA is overwritten with R, and SB is
!    overwritten with Z.
!    Output, real(kind=dp) C, S, the cosine and sine of the
!    Givens rotation.
  real(kind=dp) c
  real(kind=dp) r
  real(kind=dp) roe
...
!  Parameters:
!    Input, integer :: N, the number of entries in the vector.
!    Input, real(kind=dp) SA, the multiplier.
!    Input/output, real(kind=dp) X(*), the vector to be scaled.
!    Input, integer :: INCX, the increment between successive
!    entries of X.
  integer :: i
  integer :: incx
...
!    LC: QA214.L56.
!  Parameters:
!    Input/output, real(kind=dp) A(LDA,N).  On input, the M by N
...
!    Output, real(kind=dp) S(MM), where MM = max(M+1,N).  The first
!    min(M,N) entries of S contain the singular values of A arranged in
!    descending order of magnitude.
!    Output, real(kind=dp) E(MM), where MM = max(M+1,N).  Ordinarily
!    contains zeros.  However see the discussion of INFO for exceptions.
!    Output, real(kind=dp) U(LDU,K).  If JOBA = 1 then K = M;
...
!    Output, real(kind=dp) V(LDV,N), the N by N matrix of right singular
...
!    its superdiagonal.  Thus the singular values of A and B are the same.
  ...
  real(kind=dp) t1
  real(kind=dp) test
  real(kind=dp) u(ldu,m)
  real(kind=dp) v(ldv,n)
  logical wantu
  logical wantv
  real(kind=dp) work(m)
  real(kind=dp) ztest
...
!  Parameters:
!    Input, integer :: N, the number of entries in the vectors.
!    Input/output, real(kind=dp) X(*), one of the vectors to swap.
!    Input, integer :: INCX, the increment between successive
!    entries of X.
!    Input/output, real(kind=dp) Y(*), one of the vectors to swap.
!    Input, integer :: INCY, the increment between successive
!    elements of Y.
  integer :: i
  integer :: incx
...
!
subroutine print_reg(a,b,outu,fmt_header,fmt_trailer)
! regress b(:) on a(:,:) and print the regression coefficients
real(kind=dp)    , intent(in)           :: a(:,:) ! (m,n) LHS matrix
real(kind=dp)    , intent(in)           :: b(:)   ! (m)   RHS
integer          , intent(in), optional :: outu
character (len=*), intent(in), optional :: fmt_header,fmt_trailer
real(kind=dp)                           :: x(size(a,2)),resid(size(a,1))
integer                                 :: outu_
...
! write (outu_,*) "shape(a)=",shape(a)," shape(b)=",shape(b)," shape(x)=",shape(x) !! debug
call normal_solve_wrap(a,b,x)
if (present(fmt_header)) write (outu_,fmt_header)
write (outu_,"('coeff:',100(1x,f12.6))") x
resid = b - matmul(a,x)
write (outu_,"('SSE: ',f12.6)") sum(resid**2)
if (present(fmt_trailer)) write (outu_,fmt_header)
end subroutine print_reg
!
subroutine normal_solve_wrap(a,b,x,flag)
!! NORMAL_SOLVE_WRAP solves a linear system using the normal equations, calling NORMAL_SOLVE
real(kind=dp), intent(in)  :: a(:,:) ! (m,n) LHS matrix
real(kind=dp), intent(in)  :: b(:)   ! (m)   RHS
real(kind=dp), intent(out) :: x(:)   ! (n)   solution
integer      , intent(out), optional :: flag   ! returned zero if no error
integer                      :: flag_,m,n
character (len=*), parameter :: msg = "in normal_solve_wrap, "
flag_ = 0
m    = size(b)
n    = size(x)
if (size(a,1) /= m) then
...
if (flag_ /= 0) goto 111
call normal_solve(m,n,a,b,x,flag_)
if (flag_ == 0) return
111 continue
if (present(flag)) then
   ...
   write (*,*) msg,"flag_=",flag_,"STOPPING"
   stop
end if
end subroutine normal_solve_wrap
!
subroutine normal_solve_base(m,n,a,b,x,flag)
...
!    1, an error occurred.
integer      , intent(in)  :: m        ! # of rows of A
integer      , intent(in)  :: n        ! # of columns of A
real(kind=dp), intent(in)  :: a(m,n)   ! LHS matrix
real(kind=dp), intent(in)  :: b(m)     ! RHS
real(kind=dp), intent(out) :: x(n)     ! solution
integer      , intent(out), optional :: flag     ! returned zero if no error
real(kind=dp)              :: ata(n,n) ! will store A'A
real(kind=dp)              :: ata_c(n,n)
real(kind=dp)              :: atb(n)   ! stores A'b
integer                    :: flag_
character (len=*), parameter :: msg = "in normal_solve_base, "
flag_ = 0
if (m < n) then
   flag_ = 1
   if (present(flag)) then
...
call r8mat_cholesky_factor(n,ata,ata_c,flag_) ! out: ata_c, flag_
if (flag_ /= 0) then
   if (present(flag)) then
      flag = flag_
      return
...
call r8mat_cholesky_solve(n,ata_c,atb,x) ! out: x
end subroutine normal_solve_base
!
...
!    Output, real(kind=dp) X(N), the least squares solution.
  ...
  integer :: n
  real(kind=dp) a(m,n)
  real(kind=dp) a_copy(m,n)
  real(kind=dp) b(m)
  real(kind=dp) e(max(m+1,n))
  integer :: i
  integer :: info
  integer :: lda
  integer :: ldu
